...
TEST(TrojanMapTest, Getfunction) {
  TrojanMap m;
  m.CreateGraphFromCSVFile();
  std::string x = "20400292";
  double Lat = m.GetLat(x);
  EXPECT_EQ(Lat, 34.0273404);
  double Lon = m.GetLon(x);
  EXPECT_EQ(Lon, -118.2765547);
  std::vector<std::string> Neighbor = m.GetNeighborIDs(x);
  std::vector<std::string> exNeighbor = {"6813565320", "6813513564", "5556117120"};
  EXPECT_EQ(Neighbor, exNeighbor);
  std::string y = "150960208";
  std::string Name = m.GetName(y);
  EXPECT_EQ(Name, "Saint James Park");
...
TEST(TrojanMapTest, Autocomplete) {
  ...
  std::vector<std::string> gt4 = {"Saint James Park","Saint Agnes Elementary School", "Saint Marks Lutheran Church"};
  EXPECT_EQ(names, gt4);
}
